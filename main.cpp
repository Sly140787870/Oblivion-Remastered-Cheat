...
HRESULT APIENTRY hkPresent(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags) {
	...
	DirectX12Interface::CommandQueue = *reinterpret_cast<decltype(DirectX12Interface::CommandQueue)*>(reinterpret_cast<uintptr_t>(pSwapChain) + 0x118);
	...
	if (ShowMenu == true) 
	{
		if (ImGui::Begin("##Main"))
		{
			if (ImGui::BeginTabBar("Tab_0"))
			{
				if (ImGui::BeginTabItem("Visuals"))
				{
					ImGui::Checkbox("ESP", &Is2DESP);
				...
				if (ImGui::BeginTabItem("Aim"))
				{
					...
					ImGui::SliderFloat("FOV", &AimFOV, 1, 300);
					ImGui::SliderFloat("Distance", &AimDistance, 0, 5000);
					ImGui::EndTabItem();
				}
